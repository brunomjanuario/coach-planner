...
- **Feature**: `21-opponents` — done and verified (PASS, no fix iterations needed — clean first pass).
- **Phase / Task**: All 5 tasks complete — T1 (`opponentService` + `opponents` collection + seed, `b023205`) → T2 (v2→v3 schema migration deriving opponents from stored games — took v3 since v2 already belongs to `20-competitions`'s migration on this branch, per AC OPP-02.6; a shared `distinctGameFieldValues` helper now backs both migrations, `490f698`) → T3 (`OpponentsPopup` list + create — deliberately duplicated from `CompetitionsPopup` rather than shared, since `20` hasn't merged to `main` yet, per the task's executor note, `6698546`) → T4 (rename cascades to every game carrying the old name; a same-named standings rival row stays untouched — the one behavior this feature has that `20` doesn't, `bcddf50`) → T5 (delete behind a counted `ConfirmationPopup`, wired into a new "Opponents" header button on `Games.jsx` beside `20`'s "Competitions" button, `b26b3ec`). Games keep their existing `opponent` string (AD-010) — a delete leaves games' stored names untouched by design. Verifier: 27/27 ACs spec-matched, 4/4 discrimination-sensor mutations killed (including one confirming the rival-row-independence test actually discriminates), 1026 tests green, 0 regressions. See `.specs/features/21-opponents/validation.md`.
- **Completed**: T1–T5, all committed individually on `feature/21-opponents`, plus a docs commit marking tasks.md/spec.md status and the Verifier's validation.md.
- **In-progress** (file:line): none — feature closed out.
- **Next step**: `feature/21-opponents` is not yet merged to `main` (nor is `feature/20-competitions`, which it was branched from — `21` is not mergeable until `20` lands, or gets rebased onto `main` after `20` merges). `22-game-form-selects` is the natural next pick — it's the feature that makes the game form actually consume the competitions/opponents lists both `20` and `21` deliberately left out of scope, and it depends on both.
- **Blockers**: `feature/21-opponents` is stacked on the unmerged `feature/20-competitions` (PR #23) — merge order matters; don't merge `21` first.
- **Uncommitted files**: none — everything for `21` is committed.
- **Branch**: `feature/21-opponents`, branched from `feature/20-competitions` (not from `main` — `main` does not yet have the `competitions` collection or the v1→v2 migration this feature's v2→v3 migration builds on). Branches for this project are merged outside the session via GitHub PRs — don't assume `main` is current without checking. `main` includes features up through `19-games-three-column` (commit `afdecb8`) as of `20`'s branch creation.
- **Remaining round-two features**: `22`–`24` are specified (`spec.md` + `tasks.md`) but not started.
