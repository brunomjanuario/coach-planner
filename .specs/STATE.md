...
- **Next step**: `feature/22-game-form-selects` is not yet merged to `main`. `23-settings-tabs` and `24-profile-settings` are the two remaining specified-but-unstarted features; both are independent of `22` and of each other.
- **Blockers**: none — this branch was cut from `main` after both `20` and `21` merged, so unlike those two it is not stacked on anything unmerged.
- **Uncommitted files**: none — everything for `22` is committed.
- **Branch**: `feature/22-game-form-selects`, branched from `main` (`main` already had `20-competitions` (#23) and `21-opponents` (#24) merged — both squash-merged, so this branch's history is just its own 7 commits on top of `main`). Branches for this project are merged outside the session via GitHub PRs — don't assume `main` is current without checking.
- **Remaining round-two features**: `23`–`24` are specified (`spec.md` + `tasks.md`) but not started.
