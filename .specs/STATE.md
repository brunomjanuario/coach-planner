...
- **Feature**: `41-training-pdf-export` — **done and verified (PASS after one fix→re-verify cycle, no escalation needed).**
- **Phase / Task**: All 4 phases complete — Phase 1/T1–T3 (`src/lib/contentDisposition.js` filename parsing, `apiClient.js`'s `requestCore` split + new `apiFetchBlob`, `src/lib/download.js`'s `triggerDownload` — commits `378bb55`, `62d889b`, `cd3514b`) → Phase 2/T4–T5 (`browserTimeZone()` in `dates.js`, `trainingService.exportPdf` plus the 24-file `apiFetchBlob: vi.fn()` mock-factory sweep the new import forced — `3e5be42`, `44b4c80`) → Phase 3/T6–T7 (the Export PDF button, then its in-flight/error states — `0193a0f`, `e241b75`) → Phase 4/T8 (docs + `AD-026` — `d2ba4fd`).
...
- **Verifier**: First pass PASS overall with 3 Minor/Cosmetic gaps flagged (25/28 ACs matched spec outcome cleanly; discrimination sensor 1/3 mutations killed) — `src/lib/contentDisposition.js`'s `filename*=` branch had no traversal-stripping test (mutant survived), `TrainingDetailsPopup.jsx`'s double-click guard was unprovable by any click-based test since the button's own `disabled` attribute already blocks a second click, and `docs/07-components.md` never literally named `apiFetchBlob`. Fix commit `a96ef1a` closed all three (added the missing traversal test — confirmed it kills the exact mutant; documented the guard as intentional defense-in-depth against a same-tick double-invocation `disabled` alone can't rule out, rather than forcing an artificial test; added the literal `apiFetchBlob` reference). A scoped re-verification the same day confirmed all three closed and the gate still green at 1444/1444. See `.specs/features/41-training-pdf-export/validation.md` (includes the re-verification section).
- **Completed**: T1–T8 each committed individually on `main`, plus the Verifier's fix commit (`a96ef1a`) — no feature branch, matching how `39-backend-integration` and `40-frontend-docker` shipped.
- **In-progress** (file:line): none — feature closed out.
- **Next step**: none specified for this feature. `docs/05-services.md`'s `standingsService` row and `docs/07-components.md`'s `TrainingDetailsPopup` opening line are both pre-existing doc staleness noted but not fixed here (see Open items below) — worth a real look if a future feature touches either area.
- **Blockers**: none.
- **Uncommitted files**: none — everything for `41` is committed.
- **Branch**: `main` — this feature was executed directly on `main`, matching `39`/`40`'s precedent (see git log around `ee18f68`).
- **Open items carried forward**: all pre-`41` open items (see the superseded `28` handoff below) are unchanged by this feature — none of them touch `apiClient.js`, `trainingService.js`, or `TrainingDetailsPopup.jsx`. New from `41`: (1) `docs/05-services.md`'s `standingsService` row still describes the pre-AD-022 client-side-computed "our row" — stale, not touched here since it's outside this feature's scope; (2) `docs/07-components.md`'s `TrainingDetailsPopup` section still calls it a "read-only view" despite Delete/Rate squad/Edit/Export PDF all being present — the Export PDF paragraph was added without rewriting the stale opening line, since a full rewrite was out of scope for this feature; (3) three candidate lessons recorded from this feature's Verifier run (L-022, L-023, L-024 in `.specs/lessons.json`) via the documented no-script fallback, since `scripts/lessons.py` doesn't exist in this repo — still `candidate` status, not yet promoted.
...
- **Verified end-to-end in the browser** against a real training with 3 exercises (10min×1, 20min×2, 10min×3 reps, 80 total planned minutes): percentages 13% / 50% / 38% and Previous/Next disabled-state bounds all confirmed correct by direct interaction, not just the test suite.
- **Verifier**: Single pass, PASS with one Minor flagged gap (not a FAIL) — independently recomputed the 13%/50%/38% claim from `src/model/seed.js` and confirmed it exact; confirmed the diagram slot truly renders nothing (no dead code); confirmed `ExerciseDetailsPopup`'s local index-based Prev/Next never requires the parent to re-render; gate green at 1271/1271, discrimination sensor 3/3 mutations killed (de-weighting `plannedShare`, reverting the exercise row from `<button>` to `<div>`, removing the Previous `disabled` guard). Gap: tasks.md T3 checked off Enter-key row activation as done with no dedicated keyboard test (behavior was still correct via native `<button>` semantics). Closed immediately with a real `userEvent.keyboard("{Enter}")` test. Full suite now 1272/1272. See `.specs/features/28-training-exercise-details/validation.md` (includes the post-fix note).
- **Completed**: T1–T4 each committed individually on `feature/28-training-exercise-details`, plus the gap-closing test and a pending docs commit for the Verifier report.
- **In-progress** (file:line): none — feature closed out.
- **Next step**: per the recommended order, `29-exercise-designer` is next — the only Complex feature in round three, with its own `design.md` already written. It needs the diagram slot `28` just reserved, and it bumps React to `^19.2.0` for `react-konva`. `feature/28-training-exercise-details` is not yet merged to `main`; open its PR before branching `29`.
- **Blockers**: none. `feature/28-training-exercise-details` branches from `main` at `34a960c` (which already had `25`, `26`, `27`, `32` merged).
- **Uncommitted files**: the docs closeout for `28` (tasks.md status, validation.md, this Handoff, README.md) is pending commit.
- **Branch**: `feature/28-training-exercise-details`, branched from `main` at `34a960c`, not yet merged.
- **Remaining round-three features**: `29`, `30`, `31` (3 of 8, 21 of 39 atomic tasks — `25`, `26`, `27`, `28`, `32` are all done). Recommended order: `29` → `30` → `31` (`31` can also run any time since it's independent, but `30` consumes the `Tabs` it restyles — see `.specs/README.md`'s round-three section).
- **Open items carried forward**: (1) the `11-dashboard` batch-aggregation question on `ratingService` — still unanswered, still not forced; (2) `23`'s SETT-04.3 test-strength gap — **resolved** by `31-settings-tabs-polish` T3: `Settings.test.jsx` now spy-wraps `Tabs` (`vi.mock` with `importOriginal`) and asserts the exact `active` value Settings passes down for a bogus/missing `?tab=`, independent of `Tabs.jsx`'s own fallback; the mutation (removing the `TAB_IDS.includes` guard) was performed and observed to turn both new tests red before being reverted; (3) the `24` sign-out/refresh quirk — still accepted, untouched; (4) `TeamCard`/`PlayerCard` image paths still resolve only in dev — still unplanned; (5) `26`'s Verifier gap (route-identity-across-navigation unproven) — low severity, not assigned; (6) lesson L-013 (candidate, from `27`, corroborated by `25`): migrating N call sites to a shared component needs a variant assertion per call site; (7) process note from `25`: prefer the `Edit` tool over ad hoc Python string-splicing on test files; (8) from `32`: `src/index.css` still carries leftover Vite-template default CSS (`@layer base { button {...} }`) that silently overrides any component without an explicit Tailwind background/color utility — worth remembering for `29`'s new components; (9) `32`'s dead-code finding on `Home.jsx`'s filter-reset guard — still unreachable, still harmless, still worth a real test if Home ever gains a live refresh path; (10) **new, from `28`**: task-completion checklists (tasks.md "Done when" boxes) can be checked off for behavior that's true-but-untested (native-element semantics, browser defaults) — worth a second look at "done" boxes that cite a platform guarantee rather than app code, since that's exactly where a checklist and its actual test coverage can quietly diverge.
