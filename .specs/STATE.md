...
- **Feature**: `25-dashboard-tile-lists` — **done and verified (PASS on re-verify, after one fix→re-verify cycle).** Third round-three feature executed; the largest so far (6 tasks, Large scope).
- **Phase / Task**: All 6 tasks complete — T1 (`src/lib/dashboardStats.js`: `teamRows` sorts alphabetically case-insensitively by full display name; `upcomingRows` picks soonest-upcoming records, falling back to most-recent-past (`basis: "past"`) when nothing is upcoming — AC DTILE-01, `d2bdd18`) → T2 (`src/components/ListTile.jsx` new: count + up to N linked rows, reusing `StatTile`'s empty/loading markup — AC DTILE-01/02/03, `618b318`) → T3 (`src/pages/Teams.jsx` gains `?team=` via `useSearchParams`, mirroring `Settings.jsx`'s `?tab=`; selecting writes the param, deleting the selected team clears it, an unknown id resolves to no selection without throwing — AC DTILE-04, `e006551`) → T4 (Teams tile → `ListTile`, verified end-to-end with the real `Teams` page mounted — AC DTILE-01/03, `0c3e8ae`) → T5 (Trainings/Games tiles → `ListTile`; training data source switched `getAll()` → `getAllNumbered()` so rows can show a training number — AC DTILE-01/02/03, `945c4ad`) → T6 (proves a tile never renders a row for a record outside its own most-recently-loaded collection; cites rather than duplicates the pre-existing deleted-record 404 tests in `Trainings.test.jsx`/`Games.test.jsx` — AC DTILE-03.5, `7c76ff3`).
- **Verifier**: **First pass FAIL** (all 3 gaps ranked Minor — implementation independently confirmed correct in every case, these were test-discrimination/coverage gaps, not shipped defects): (1) a surviving mutant in `teamRows`'s case-insensitive sort — the original test fixture (`zeta`/`Alpha`/`Beta`) never crossed a case boundary that would flip order under raw comparison; (2) no test proved the team filter narrows Trainings/Games `ListTile` rows specifically; (3) no test clicked a Games-tile row to confirm its popup opens. Gaps 2 and 3 turned out to be tests that had already been written during T5 but were destroyed by an overly-greedy cleanup script mid-session (a `rindex` search that cut into the wrong test's boundary) — recovered, not freshly authored. **Fixed in `edee6f5`**: a cross-case fixture (`'Cherry'` vs `'banana'`) that actually discriminates the mutation, plus the two recovered tests. **Re-verify: PASS** — all 3 mutations re-run and confirmed killed, no new gaps, full suite 1226/1226 stable across two consecutive runs, lint/build clean. See `.specs/features/25-dashboard-tile-lists/validation.md` (first-pass report + appended re-verify section).
- **Completed**: T1–T6 each committed individually on `feature/25-dashboard-tile-lists`, plus the gap-closing commit (`edee6f5`) and a pending docs commit for both validation passes.
- **In-progress** (file:line): none — feature closed out.
- **Next step**: per the recommended order, `32-dashboard-filter-ui` is next (it sizes the dashboard tiles and must follow `25`, which just changed what they contain). `feature/25-dashboard-tile-lists` is not yet merged to `main`; open its PR before branching `32`.
- **Blockers**: none. `feature/25-dashboard-tile-lists` branches from `main` at `62c71d2` (which already had `26` and `27` merged).
- **Uncommitted files**: the docs closeout for `25` (tasks.md status, validation.md, this Handoff, README.md) is pending commit.
- **Branch**: `feature/25-dashboard-tile-lists`, branched from `main` at `62c71d2`, not yet merged.
- **Remaining round-three features**: `28`–`32` (5 of 8, 24 of 39 atomic tasks — `25`, `26`, `27` are all done). Recommended order: `32` → `28` → `29` → `30` → `31` (see `.specs/README.md`'s round-three section for the dependency reasoning; `32` sizes tiles and must follow `25`).
- **Open items carried forward**: (1) the `11-dashboard` batch-aggregation question on `ratingService` — still unanswered, still not forced; (2) `23`'s SETT-04.3 test-strength gap — assigned to `31-settings-tabs-polish` T3, not yet executed; (3) the `24` sign-out/refresh quirk — still accepted, untouched; (4) `TeamCard`/`PlayerCard` image paths still resolve only in dev — still unplanned; (5) `26`'s Verifier gap (route-identity-across-navigation unproven by a direct test) — low severity, not assigned; (6) lesson L-013 (candidate, from `27`): migrating N call sites to a shared component needs a variant assertion per call site — now corroborated by `25`'s own near-miss (two such tests were lost to a cleanup-script bug and only caught by the Verifier's coverage check, not by the author); (7) **process note, not a product item**: mid-session test-file edits via ad hoc Python string-splicing are risky — a `rindex`-based "find the start of this test block" search silently deleted an adjacent unrelated test twice in this feature's history. Prefer the `Edit` tool's exact-string replacement over scripted text surgery on test files going forward.
