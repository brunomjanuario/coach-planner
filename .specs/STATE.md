...
- **Feature**: `32-dashboard-filter-ui` — **done and verified (PASS, two flagged Minor gaps closed immediately, no formal re-verify cycle needed).** Fourth round-three feature executed; the last one touching the dashboard.
- **Phase / Task**: All 4 tasks complete — T1 (`src/components/Tile.jsx`: `TILE_CLASS` gains `min-h-36`, hand-measured against `LeaderTile`'s tallest real variant, so every tile in both sections is one height instead of two independently-sized rows — verified by eye in the browser with an 8-way tied Most Goals list stretching its whole row — AC DFILT-01, `c6bdc8d`) → T2 (`src/components/TeamFilterBar.jsx` new: toggle-button chip bar, `role="group"` + `aria-pressed` per chip, not tabs — AC DFILT-02, `0629977`) → T3 (chip bar replaces the bare `<select>` on the dashboard; 10 existing tests mechanically converted from `selectOptions` to chip clicks — AC DFILT-02, `afd2d63`) → T4 ("Showing: <team>" signpost + ghost `Clear` action, plus a defensive `useEffect` falling back to "All teams" if the filtered team disappears — AC DFILT-03, `2f326e2`).
- **Real bug found and fixed during browser verification** (not by the test suite): `TeamFilterBar`'s unpressed chips had no explicit `bg-` utility, so the app's leftover Vite-template `@layer base { button { background-color: #1a1a1a } }` global style bled through, rendering them near-black instead of blending into the track. Fixed with `bg-transparent` in the same commit (T3).
- **Verifier**: Single pass, PASS with two Minor flagged gaps (not a FAIL) — 20/22 criteria matched spec outcome exactly, gate green at 1246/1246, discrimination sensor 2/3 mutations killed (dropping `min-h-36`; breaking chip `aria-pressed` exclusivity). The third mutation — removing the defensive filter-reset `useEffect` — **survived**, and the Verifier correctly diagnosed why: `Home.jsx` has no live mid-session refetch, so a fresh mount already starts unfiltered regardless of the guard's existence: the guard's branch is currently unreachable dead code, though its own logic was confirmed correct by direct source read and it's kept as harmless, spec-required, forward-looking code. Also flagged: no dedicated Tab-reachability test for the chip bar. Both closed in `1b902e7` — a real Tab-order test added, and the dangling-team test's comment rewritten to state plainly (not imply) what it actually proves given the architecture. Full suite now 1247/1247. See `.specs/features/32-dashboard-filter-ui/validation.md` (includes the post-fix note).
- **Completed**: T1–T4 each committed individually on `feature/32-dashboard-filter-ui`, plus the gap-closing commit (`1b902e7`) and a pending docs commit for the Verifier report.
- **In-progress** (file:line): none — feature closed out.
- **Next step**: per the recommended order, `28-training-exercise-details` is next (opens the door for `29-exercise-designer`, which needs its reserved diagram slot). `feature/32-dashboard-filter-ui` is not yet merged to `main`; open its PR before branching `28`.
- **Blockers**: none. `feature/32-dashboard-filter-ui` branches from `main` at `5d8f6f0` (which already had `25`, `26`, `27` merged).
- **Uncommitted files**: the docs closeout for `32` (tasks.md status, validation.md, this Handoff, README.md) is pending commit.
- **Branch**: `feature/32-dashboard-filter-ui`, branched from `main` at `5d8f6f0`, not yet merged.
- **Remaining round-three features**: `28`, `29`, `30`, `31` (4 of 8, 15 of 39 atomic tasks — `25`, `26`, `27`, `32` are all done). Recommended order: `28` → `29` → `30` → `31` (see `.specs/README.md`'s round-three section; `31` can also run any time since it's independent, but `30` consumes the `Tabs` it restyles).
- **Open items carried forward**: (1) the `11-dashboard` batch-aggregation question on `ratingService` — still unanswered, still not forced; (2) `23`'s SETT-04.3 test-strength gap — assigned to `31-settings-tabs-polish` T3, not yet executed; (3) the `24` sign-out/refresh quirk — still accepted, untouched; (4) `TeamCard`/`PlayerCard` image paths still resolve only in dev — still unplanned; (5) `26`'s Verifier gap (route-identity-across-navigation unproven by a direct test) — low severity, not assigned; (6) lesson L-013 (candidate, from `27`, corroborated by `25`): migrating N call sites to a shared component needs a variant assertion per call site; (7) process note from `25`: prefer the `Edit` tool over ad hoc Python string-splicing on test files — a `rindex`-based search destroyed adjacent tests twice in that feature's history; (8) **new, from `32`**: the app still carries the leftover Vite-template default CSS in `src/index.css` (`@layer base { button { ... } }`, `:root` dark-mode colors) — it silently overrides any component that doesn't set an explicit Tailwind background/color utility. Not worth a dedicated cleanup feature on its own, but worth remembering when building any new bare `<button>`-based component; (9) the `32` Verifier's dead-code finding: `Home.jsx`'s defensive team-filter-reset `useEffect` is currently unreachable given the mount-once data model — harmless, but if `29-exercise-designer` or any future feature gives Home a live refresh path, this is the first place that becomes newly testable and worth a real test then.
