...
- **Feature**: `27-popup-button-system` — **done and verified (PASS after closing one flagged gap, no formal re-verify cycle needed).** Second round-three feature executed.
- **Phase / Task**: All 5 tasks complete — T1 (`src/components/Button.jsx` new: four variants — primary/secondary/danger/ghost — each hand-verified against the 4.5:1 WCAG AA floor and recorded in a doc comment; `focus-visible` ring, `disabled` state that actually blocks the click handler, `type`/`form` forwarding, unknown-variant fallback to secondary — AC BTN-01, `08b127e`) → T2 (`src/components/PopupActions.jsx` new: destructive children left via `mr-auto`, the rest right-aligned in one wrapping row — AC BTN-02, `fb38e1a`) → T3 (`GameResultPopup`'s five-button/two-row/five-colour footer — Clear Result red-600, Cancel grey-on-white, Save blue, Rate squad green, Delete Game alone on a second row in red-800 — collapsed to one row: both destructive actions as `danger`, Rate squad as `secondary`; `GameSavePopup` migrated too — AC BTN-02/03, `99b47fc`) → T4 (the remaining 10 files — `TrainingDetailsPopup`, `TrainingSavePopup`, `ExerciseFields`, `TeamPopup`, `PlayerPopup`, `SquadRatingPopup`, `RivalRowPopup`, `OpponentsPopup`, `CompetitionsPopup`, `ConfirmationPopup` — migrated to `Button`/`PopupActions`; found and fixed `ConfirmationPopup`'s inverted colours (Cancel was `bg-red-500`, Submit was `bg-green-500`) to Cancel=secondary/Submit=danger — AC BTN-04, `26513b9`) → T5 (a source-scan guard in `Button.test.jsx` asserting `bg-gray-300 text-white` never appears in `src/` again, excluding `__tests__`; verified by actually reintroducing the pattern into `Sidebar.jsx`, confirming the suite went red naming that file, then reverting — AC BTN-01.6, `da025ef`).
- **Verifier**: Single pass, PASS with one minor flagged gap (not a FAIL, so no formal fix→re-verify cycle) — 17/19 criteria matched spec outcome exactly with `file:line` evidence, gate green at 1175/1175, discrimination sensor 3/3 mutations killed (grey-on-white regression, GameResultPopup's two-row split, disabled-but-still-firing onClick). Gap: 8 of the 11 migrated popups (all but GameResultPopup/GameSavePopup/TrainingDetailsPopup) had correct variant assignments confirmed only by direct source read, not by a test assertion — closed immediately in commit `de85f52` (9 new tests, one per file, covering all footer and inline-rename buttons plus SquadRatingPopup's and ExerciseFields' two-state footers). Full suite now 1184/1184. See `.specs/features/27-popup-button-system/validation.md` (includes the post-fix note).
- **Completed**: T1–T5 each committed individually on `feature/27-popup-button-system`, plus the gap-closing commit (`de85f52`) and the Verifier-report docs commit.
- **In-progress** (file:line): none — feature closed out.
- **Next step**: per the recommended order, `25-dashboard-tile-lists` is next. `feature/27-popup-button-system` is not yet merged to `main`; open its PR before branching `25`.
- **Blockers**: none. `feature/27-popup-button-system` branches from `main` at `a5d2294` (which already had `26` merged via #28).
- **Uncommitted files**: none — everything for `27` is committed.
- **Branch**: `feature/27-popup-button-system`, branched from `main` at `a5d2294`, not yet merged.
- **Remaining round-three features**: `25`, `28`–`32` (6 of 8, 30 of 39 atomic tasks — `26` and `27` are both done). Recommended order: `25` → `32` → `28` → `29` → `30` → `31` (see `.specs/README.md`'s round-three section for the dependency reasoning; `27` being done means `28`/`29`/`30` can now build popups on the shared `Button` from the start, no second migration needed).
- **Open items carried forward**: (1) the `11-dashboard` batch-aggregation question on `ratingService` — still unanswered, still not forced; (2) `23`'s SETT-04.3 test-strength gap — assigned to `31-settings-tabs-polish` T3, not yet executed; (3) the `24` sign-out/refresh quirk — still accepted, untouched; (4) `TeamCard`/`PlayerCard` image paths still resolve only in dev — still unplanned; (5) `26`'s Verifier gap (route-identity-across-navigation unproven by a direct test) — low severity, not assigned; (6) a project-local lesson (L-013, candidate) was recorded during `27`'s verification: when a feature migrates N call sites to a shared component, each call site needs its own variant/class assertion, not just the shared component's own tests — relevant to `28`/`29`/`30`'s upcoming popup work.
