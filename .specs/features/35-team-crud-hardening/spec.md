...
| Label association pattern | `useId()`-generated ids matching `TrainingSavePopup`/`GameSavePopup`'s existing convention (`htmlFor={fieldId}` / `id={fieldId}` pairs) | Already the established pattern for every other form-bearing popup in this codebase; no new convention introduced. | y |
