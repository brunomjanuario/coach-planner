...
### T2: Configure Vitest in the Vite config
...
- [ ] `test.globals` is `true` so `describe`/`it`/`expect` need no import
- [ ] `test.setupFiles` points at `src/test/setup.js`
- [ ] Config still exports a single `defineConfig` call; `npm run build` unaffected

**Tests**: none (matrix: build config → none)
**Gate**: build

**Commit**: `chore(test): configure vitest with jsdom environment`

---

### T3: Create the test setup file
...
- [ ] `afterEach` calls RTL `cleanup()`
- [ ] `afterEach` calls `localStorage.clear()` — required by `01-persistence-layer`, which would otherwise leak state across test files
- [ ] File is referenced by `vite.config.js` from T2

**Tests**: none (matrix: build config → none) — its correctness is proven by T5
**Gate**: build

**Commit**: `chore(test): add vitest setup with jest-dom and cleanup`

---

### T4: Add test npm scripts
...
**Done when**:
- [ ] `npm test` maps to `vitest run` (single-shot, CI-safe — **not** watch mode)
- [ ] `npm run test:watch` maps to `vitest`
- [ ] `npm test` on the current tree exits **zero** with "no test files found" (AC TEST-01.3)
- [ ] A deliberately failing scratch test makes `npm test` exit non-zero; scratch test deleted before commit

**Tests**: none (matrix: build config → none)
**Gate**: build

**Commit**: `chore(test): add test scripts`

---

### T5: Prove the harness with a Sidebar component test
...
**Done when**:
- [ ] Renders `Sidebar` wrapped in `MemoryRouter` and `AuthProvider`
- [ ] Asserts all six nav links resolve to the correct `href` (`/`, `/teams`, `/trainings`, `/games`, `/calendar`, `/settings`)
- [ ] Asserts clicking logout calls `signOut` and clears the `user` key from `localStorage`
- [ ] Two tests in the file do not leak DOM state into each other (AC TEST-04.3) — assert an empty container at the start of the second
- [ ] Establishes the co-location pattern (`__tests__/` beside source) every later feature follows
...
**Commit**: `test(sidebar): add first component test proving the harness`

---

### T6: Clear the lint errors for a green baseline
...
**Done when**:
- [ ] `AuthContext.jsx:29` — the unused `password` parameter in `signUp` is resolved. **Keep the parameter** (the signature is the seam a real backend will use); silence it via the config's existing `varsIgnorePattern` by renaming to `_password`
- [ ] `AuthContext.jsx:52` — the `react-refresh/only-export-components` warning is resolved by moving `useAuth` into `src/context/useAuth.js`, updating all 5 import sites
- [ ] `Calendar.jsx:82` — the unused `monthName` variable is deleted (`displayMonth` is the one actually rendered)
- [ ] `index.html` — the dead `<link href="/src/styles.css">` is removed, silencing the build-time warning (AC TEST-05.2)
- [ ] `npm run lint` reports **0 errors, 0 warnings**
- [ ] `npm run build` emits no "doesn't exist at build time" warning
- [ ] T5's test still passes after the `useAuth` move
- [ ] Full gate passes: `npm run lint && npm run build && npm test`
- [ ] Test count: 3 tests pass (no change from T5)

**Tests**: component (T5's tests must survive the `useAuth` refactor)
**Gate**: build

**Commit**: `fix(lint): clear all lint errors and dead stylesheet link`
